...
export class CellularAutomaton {
  ...
  yLessThanZero(pixel) {
    if (pixel.y < 0) {
      return Pixel.XY(
        pixel.x,
        this.grid.gridHeight - (abs(pixel.y) % this.grid.gridHeight)
      )
    }
    return pixel
  }
  ...
  xLessThanZero(pixel) {
    if (pixel.x < 0) {
      return Pixel.XY(
        this.grid.gridWidth - (abs(pixel.x) % this.grid.gridWidth),
        pixel.y
      )
    }
    return pixel
  ...
  isActive(x, y) {
    ...
    return (
      red(activationColor) == red(pixelColor) &&
      green(activationColor) == green(pixelColor) &&
      blue(activationColor) == blue(pixelColor) &&
      alpha(activationColor) == alpha(pixelColor)
    )
  }
}
